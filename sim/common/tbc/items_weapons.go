...
func init() {
	...
	// https://www.wowhead.com/tbc/item=12798
	core.NewItemEffect(12798, func(agent core.Agent) {
		character := agent.GetCharacter()

		annihilatorDebuffAuras := character.NewEnemyAuraArray(func(target *core.Unit) *core.Aura {
		...
		annihilatorSpell := character.GetOrRegisterSpell(core.SpellConfig{
			ActionID:    core.ActionID{SpellID: 16928},
			...
			RelatedAuraArrays: annihilatorDebuffAuras.ToMap(),
		})

		procTrigger := character.MakeProcTriggerAura(core.ProcTrigger{
			Name:       "Annihilator",
			ActionID:   core.ActionID{ItemID: 12798},
			Callback:   core.CallbackOnSpellHitDealt,
			ProcMask:   core.ProcMaskMeleeMH,
			Outcome:    core.OutcomeLanded,
			ProcChance: 0.03,
			Handler: func(sim *core.Simulation, spell *core.Spell, result *core.SpellResult) {
				annihilatorSpell.Cast(sim, result.Target)
			},
		})

		character.ItemSwap.RegisterProc(12798, procTrigger)
	})
